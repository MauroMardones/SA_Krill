...
acf_combined <- cowplot::plot_grid(p1, p2, p3, p4, p5, ncol = 1)
...
difrmse <- ggplot(combined_rmse_delta, aes(x = Scenario, y = Delta, fill = Scenario)) +
  ...
  facet_wrap(~Metric) +
  scale_fill_manual(values = c("s1.2" = "#f4a582", "s1.3" = "#bababa", "s1.4" = "#404040", "s1.5"= "#0571b0")) +
...
hc_all <- bind_rows(
  ...
  mutate(Scenario = factor(Scenario, levels = c("s1.1", "s1.2", "s1.3", "s1.4")),
...
p_hcbias <- ggplot(hc_all, aes(x = Scenario, y = Rho)) +
  ...
  scale_fill_manual(values = c("s1.1" = "#ca0020", "s1.2" = "#f4a582",
                               "s1.3" = "#bababa", "s1.4" = "#404040"), guide = "none") +
  labs(x = "", y = "Mohn's rho") +
  theme_few() +
...
scen_order <- c("s1.1", "s1.2", "s1.3", "s1.4")
...
mase_summary <- mase_all |>
  ...
  mutate(Scenario = factor(Scenario, levels = rev(c("s1.1", "s1.2", "s1.3", "s1.4")))) |>
  group_by(Scenario) |>
...
full_grid <- tidyr::expand_grid(
  ...
  Scenario = factor(c("s1.1", "s1.2", "s1.3", "s1.4"),
                     levels = c("s1.1", "s1.2", "s1.3", "s1.4")),
