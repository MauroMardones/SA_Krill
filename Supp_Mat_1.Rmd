...
```{r AKL, fig.width=5, fig.height=4, fig.cap="Representation of ALK Matrix to krill in 48.1"}
...
```{r chlindex, fig.height=3, fig.width=8, fig.cap="Interannual variability of the Chlorophyll Variability Index from 2000 to 2020. Positive anomalies (in red) and negative anomalies (in black) indicate below-average conditions"}
chl <- dat1$envdat |>
...
```{r dataserie, fig.width=5, fig.height=8, fig.cap="Data series used in krill modelling in 48.1 Subarea"}
...
kbl_pdf(scenario_table,
        caption = "Scenarios used for modelling dynamics in krill",
...
```{r selectivity, fig.height=7, fig.width=6.5, fig.cap="Selectivity by fleet in each scenario"}
...
```{r index2, fig.height=7, fig.width=6.5, fig.cap= "Standardized indices of krill abundance used as input in four model scenarios (s1.1 to s1.4), representing fishery-dependent (FISHERY) and fishery-independent (SURVEY) data across five spatial strata: Bransfield Strait (BS), Elephant Island (EI), Gerlache Strait (GS), Joinville Island (JOIN), and South West (SW). Scenario s1.4 also incorporates a predator index (PREDATOR), reflecting the integration of ecosystem variables into the assessment framework"}
...
get_pop_unc <- function(model, modelname) {
  ...
  dplyr::bind_rows(
    ...
    extract_dq("TotBio",  "Total Biomass (t)"),
...
```{r cumsum, fig.cap="Summary of estimation of different populations variables" }
...
```{r recdev, eval=FALSE, message=FALSE}
# 
...
kbl_pdf(combined_rmse,
        caption = "Combined RMSE values by scenario for abundance indices and length compositions")
```
...
```{r retrossb, fig.height=7, fig.width=6.5,, fig.cap="Retrospective analysis for spawning biomass by scenario in krill"}
par(mfrow=c(2,2), mar=c(5,4,1,1)) 
...
```{r retrof, fig.height=7, fig.width=6.5, fig.cap="Retrospective analysis for fishing mortality by scenario in krill"}
par(mfrow=c(2,2), mar=c(5,4,1,1)) 
retro1.1 <- SSplotRetro(retroSummary1.1,
            add=T,
            forecast = F,
...
kbl_pdf(tablebias01,  caption = "Mohn's rho — SSB, scenario s1.1")
kbl_pdf(tablebias01a, caption = "Mohn's rho — F, scenario s1.1")

# model2
tablebias2  <- SShcbias(retroSummary1.2, quant = "SSB", verbose = F)
tablebias2a <- SShcbias(retroSummary1.2, quant = "F",   verbose = F)
kbl_pdf(tablebias2,  caption = "Mohn's rho — SSB, scenario s1.2")
kbl_pdf(tablebias2a, caption = "Mohn's rho — F, scenario s1.2")

# model3
tablebias3  <- SShcbias(retroSummary1.3, quant = "SSB", verbose = F)
tablebias3a <- SShcbias(retroSummary1.3, quant = "F",   verbose = F)
kbl_pdf(tablebias3,  caption = "Mohn's rho — SSB, scenario s1.3")
kbl_pdf(tablebias3a, caption = "Mohn's rho — F, scenario s1.3")

# model4
tablebias4  <- SShcbias(retroSummary1.4, quant = "SSB", verbose = F)
tablebias4a <- SShcbias(retroSummary1.4, quant = "F",   verbose = F)
kbl_pdf(tablebias4,  caption = "Mohn's rho — SSB, scenario s1.4")
kbl_pdf(tablebias4a, caption = "Mohn's rho — F, scenario s1.4")
...
### Test formal de tendencia en recruitment deviates (Merino et al. 2022) 
...
p_params <- ggplot() +
  ...
  labs(x = "Valor del parámetro", y = "Densidad",
       caption = "x = estimado (phase>0) pero sin SD (Hessiano no invertido)") +
  theme_few() +
  theme(
