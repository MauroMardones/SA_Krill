...
```{r index2,  fig.cap= "Standardized indices of krill abundance used as input in four model scenarios (s1.1 to s1.4), representing fishery-dependent (FISHERY) and fishery-independent (SURVEY) data across five spatial strata: Bransfield Strait (BS), Elephant Island (EI), Gerlache Strait (GS), Joinville Island (JOIN), and South West (SW). Scenario s1.4 also incorporates a predator index (PREDATOR), reflecting the integration of ecosystem variables into the assessment framework"}
...
totbio %>%
  ...
  pivot_wider(names_from = Model, values_from = Bio_all) %>%
  mutate(across(-Yr, ~ round(. / 1e6, 1)))
...
totbio %>%
  ...
  geom_point(size = 1.5) +
  labs(x = "Año", y = "Biomasa total (Mt)",
       title = "Biomasa total estimada — subárea 48.1",
       color = "Modelo") +
  scale_color_manual(values = c("s1.1" = "red3", "s1.2" = "salmon",
                                 "s1.3" = "gray60", "s1.4" = "gray20")) +
  theme_bw()
```


Difference between scenarios regarding to summary in populations variables in Table \@ref(tab:varcompar) for details.
...
```{r cumsum, fig.cap="Comparsion in sd long term time series forecasting"}
errt <- data.frame(
...
comsum
ggsave(filename = "Figs/COMSUM.png", plot = comsum, width = 10, height = 4, dpi = 300)
```
...
```{r recpro, fig.cap="Left panel: Stock–recruitment relationships for Antarctic krill under four assessment scenarios. Predicted recruitment as a function of spawning stock biomass (SSB), with fitted Beverton-Holt curves and annual labels for selected years. Right panel: Recruitment per unit of SSB plotted against relative SSB (SSB/SSBo), illustrating differences in per-capita productivity across models ( s1.1 (no ecosystem variables), s1.2 (with predator effects), s1.3 (with environmental variables), and s1.4 (with both predator and environmental effects))"}
ggarrange(srr_sce,
          prod,
          ncol = 2)
```
...
```{r pearson-trend, fig.cap="Pearson residual trend by scenario and fleet"}
...
ggplot(df_all, aes(x = Yr, y = Pearson)) +
  ...
  geom_text(data = lm_stats,
            aes(x = min(df_all$Yr) + 2,
                y = max(df_all$Pearson, na.rm = TRUE) - 0.3,
