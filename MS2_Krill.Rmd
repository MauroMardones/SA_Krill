...
The comparative evaluation of the four model scenarios (s1.1–s1.4) revealed key differences in model performance and complexity (Table \@ref(tab:diagnostics)). Scenario s1.1, the simplest model with no ecosystem covariates, exhibited the lowest maximum gradient component (2.4e−05), suggesting a well-converged solution. It also used the fewest estimated parameters (94) and achieved a relatively low RMSE (0.88), indicating a good fit to the data despite its simplicity. However, its negative log-likelihood (NLL = 1,291.04) was higher compared to more complex models. Model s1.2, which includes only environmental covariates, showed a substantial increase in the likelihood fit (NLL = 25,963.90), although it also required a larger number of parameters (152). Similarly, s1.3, which includes only predator effects, maintained a moderate parameter count (140) but performed slightly worse in terms of RMSE (0.93), indicating potential overfitting or reduced explanatory power relative to model complexity. The full model, s1.4, incorporating both environmental and predator effects, achieved a strong likelihood value (NLL = 25,188.80), albeit with the highest number of parameters (175). Its RMSE (0.89) was similar to the other scenarios, suggesting that the inclusion of additional covariates may have improved likelihood without substantial overfitting. The gradient component (5.18e−05) remained within acceptable bounds, supporting convergence. These results suggest a trade-off between model complexity and fit, where models incorporating ecological covariates (s1.2 and s1.4) improved the likelihood function but required more parameters, while the simpler model (s1.1) retained strong convergence properties with fewer degrees of freedom.
...
flextable(diagnostics) %>%
  set_caption("diffenrent statistical componentes to evaluate perfomances and diagnostic of model scenarios") %>%
  autofit() %>%
  theme_booktabs() %>%
  fontsize(size = 10, part = "all") %>%
