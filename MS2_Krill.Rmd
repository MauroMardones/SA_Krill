...
As @methot2020stock establish in SS3 manual, natural mortality attributable to predation (M2) can be incorporated through different types of observational data, each informing distinct aspects of predator-prey dynamics. First, total mortality due to predation can be input as a "discard" component in the data structure. Since M2 represents a proportion of total mortality (Z), the quantity of individuals removed by predators can be estimated using the relative contribution of M2 to Z, i.e., `M2 / Z`. This allows the model to estimate the biomass consumed by predators, analogous to how catch is calculated for fishing fleets. Second, predator abundance can be input as an index of effort, treating M2 as a fishing mortality analog. This approach uses the same structure as a fishery-independent survey, where predator biomass serves as an index of predation pressure over time. In this framework, the predation rate is proportional to predator abundance scaled by a catchability coefficient \( Q \), following the equation:

$$
M2_t = Q * I_t
$$
where \( M2_t \) is the predation mortality in year \( t \), \( I_t \) is the predator abundance index in year \( t \), and \( Q \) is an estimated or fixed catchability parameter linking the predator index to predation pressure. Finally, information on the size or age composition of predated individuals, such as those derived from gut content analyses, can be used to inform the selectivity pattern of M2. These data are input analogously to length- or age-composition observations from fisheries, and allow the model to estimate the predation selectivity curve, capturing how predation probability varies with prey size or age.

Regarding krill predators, we will incorporate both biomass and dietary data from the three main penguin colonies —Adélie (*Pygoscelis adeliae*), Chinstrap (*Pygoscelis antarcticus*), and Gentoo (*Pygoscelis papua*) —into the stock assessment model through two complementary pathways.. First, the abundance of these predator colonies will be treated as an index of predation "effort" exerted on the krill population, allowing estimation of time-varying predation mortality rates. Second, empirical length-frequency distributions of krill derived from the diet composition of each penguin species will be used to parameterize the size-selective nature of M2, enabling the estimation of predator-specific selectivity functions. This dual approach facilitates a temporally and size-structured characterization of M2, consistent with ecosystem-based stock assessment principles and improving the ecological realism of krill population dynamics modeling. The total natural mortality (M) will thus explicitly include the predation component, expressed as \( M = M_1 + \sum M_2 \) [@methot2020stock], where \( M_1 \) represents background (non-predation) natural mortality, and \( \sum M_2 \) accounts for mortality attributed to each predator type. Consequently, total mortality is represented as \( Z = M_1 + M_2 + F \), incorporating both natural and fishing sources of mortality.
...
Regarding the performance and comparison between scenarios, @Carvalho2017 and @Carvalho2021b propose a structured approach for diagnosing stock assessment models to ensure their reliability and robustness. This diagnostic process was applied to the krill stock assessment and involved some key steps. First, convergence analysis was conducted to evaluate whether the estimation algorithm reached a stable solution. This included reviewing parameter trace plots, gradients, and the Hessian matrix to ensure that all parameters were well-estimated. Next, model fit to data was assessed by comparing observed and predicted values across key data sources, including catch, survey indices, and length composition. Residual plots were examined to detect potential patterns indicative of model misspecification. A retrospective analysis was performed by sequentially removing recent years of data (peeling) and re-estimating the model, allowing for the calculation of retrospective bias (*Rho*) for key metrics such as Spawning Stock Biomass (SSB) and Fishing Mortality (F). The model's forecasting capability was evaluated through one-step-ahead predictions and their comparison to observed values in recent years, helping identify potential bias or overfitting in recruitment or abundance estimates. Sensitivity analyses were conducted by varying ecosystem variables to assess their influence on model outputs. Likelihood profiling was also carried out to quantify parameter uncertainty and identify the influence of priors. Finally, prediction skill was assessed using hindcasting techniques to evaluate the model’s ability to reproduce historical stock dynamics.Four estimation models were developed depending on how the ecosystem components was integrated where it was included as an additional covariate within the stock-recruitment (S-R) relationship. To evaluate statistical differences in model performance, we analyzed the Root Mean Squared Error (RMSE) values across four scenario configurations (`s1.1`, `s1.2`, `s1.3`, and `s1.4`). The RMSE values were extracted from the variance tuning checks of each scenario and compiled into a data frame for statistical analysis. To test for overall differences among scenarios, a one-way ANOVA [@REF] was conducted, treating RMSE values as the dependent variable and scenario configurations as the categorical factor. If the ANOVA indicated significant differences, a post-hoc Tukey test would be performed to determine which scenario comparisons were driving the differences. By systematically applying these diagnostic steps, stock assessment models can be rigorously evaluated, ensuring they provide robust and unbiased estimates.
...
```{r popvar, eval=TRUE, fig.cap="Comparisons of krill population variables under scenarios of increasing ecosystem complexity. Metrics include total biomass (Bio_all), summary biomass (Bio_smry), recruitment (Recruit_0), and spawning biomass (SpawnBio)."}
knitr::include_graphics('Figs/unnamed-chunk-65-1.jpeg')
```

Figure \@ref(fig:compara) shows the time series of estimated spawning output for Antarctic krill under four modeling scenarios from 1990 to 2020. The reference model (blue), which excludes ecosystem components, consistently estimates higher spawning output values over time, with a steep increase observed after 2015. In contrast, the models that incorporate ecosystem components—predator data (green), environmental data (yellow), or both (red)—estimate consistently lower spawning output throughout the time series. The red line, representing the model with both predator and environmental data, remains the most conservative, suggesting that ecosystem-informed models lead to more precautionary estimates of reproductive potential. The rigth panel displays the posterior density distributions of unfished spawning biomass (SSB~o~) for the same scenarios. The reference model estimates the highest SSBo values (mode ~ 3.2), while the ecosystem-informed models estimate considerably lower SSBo, with the most constrained distribution occurring under the full ecosystem model (red). The shift in the distributions indicates that including ecosystem drivers reduces estimates of unfished reproductive capacity, which has implications for reference point calculations and harvest strategies.

```{r compara,eval=TRUE, out.width="60%", fig.cap="Estimated R0 parameter for Antarctic krill under four assessment scenarios. Models including predator and environmental data"}
knitr::include_graphics('Figs/COM1compare2_spawnbio_uncertainty.png')

```

The different configurations to estimated posterior distribution of Natural log of virgin recruitment level (R0) suggesting that the choice of covariates has a substantial impact on the estimation of unfished recruitment (Figure \@ref(fig:compara2)). The model with both environmental and predator data yields the highest R0 values, while the model with only environmental data results in the lowest estimates. This highlights the sensitivity of the recruitment parameter to assumptions about ecological drivers in the stock-recruitment relationship.

```{r compara2, eval=TRUE, out.width="60%", fig.cap="Estimated spawning output for Antarctic krill under four assessment scenarios"}
knitr::include_graphics('Figs/COM1compare16_densities_SR_LN(R0).png')
