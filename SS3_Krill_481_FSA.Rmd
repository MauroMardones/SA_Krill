...
author:
  ...
  - George Watters^[Ecosystem Antarctic Division, Southwest Fisheries Science Center, NOAA, USA]
...
Source data and temporal scale used in this stock assessment is showed in Figure \@ref(fig:datas2). Data handling for modeling purposes was elaborated with *tidyverse* library  and dependences [@Wickham2019]. 
...
Since we don't have age data monitoring program, we plan to explore *platoon* (Figure \@ref(fig:platoon)) alternative to compare its performance and results with the current length-based model using currently in WG-SAM-2024/26.
...
According to the observations made at @CCAMLRWGSAM2024 regarding changes in net size and trawl type that could affect selectivity, we have incorporated a scenario (`s9`) with a selectivity block for krill. The inclusion of selectivity blocks aims to, on the one hand, reduce the effect of autocorrelation in the model fitting process, and on the other hand, explicitly account for fleet behavior in exploring new extraction areas, whether in bathymetric or latitudinal terms. According to the data identified, a shift from traditional to continuous trawling occurred in 2006. As a result, two blocks have been established (1980-2006 and 2007-2020) as improvements. This is compared with the base scenario.
...
A comparative analysis of models `s01`, `s1`, and `s7` will be conducted, as they incorporate elements such as aggregate data, spatially structured data, and different sets of life history parameters according to @Mardones2024par. Additionally, to address the observations, scenarios `s8` and `s9` have been added and compared. The details of each observation made in the @CCAMLRWGSAM2024 report have been thoroughly addressed, and each response can be reviewed in **APPENDIX 1**.
...
Regarding the fitting of indices in general, the model attempts to interpret the trends but encounters some difficulties in effectively fitting all sources of information, as shown in (Figure \@ref(fig:resall)).
...
A selectivity block in an alternative scenario (`s9`) was implemented and compared its performance with the base model (`s1`). The comparison of the main evaluation results with both scenarios is presented in the estimates of the population variables of recruit, spawning biomass, relative biomass and virgin biomass  (Figure \@ref(fig:comparisionselec)). It can be observed that there are no significant differences, at least in the state variables.
...
- This work is considered a continuous inductive analytical exercise, which takes into account elements influencing krill dynamics, such as data, factors, opinions, and others, and integrates them through a mathematical process. This exercise is iteratively refined with observations and corrections as it is communicated to the Antarctic community. Therefore, we appreciate the feedback provided in the WG-SAM and WG-EMM 2024, which has contributed to its improvement.
