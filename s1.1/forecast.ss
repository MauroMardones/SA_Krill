...
0.75 # SPR target (e.g. 0.40)
0.75 # Biomass target (e.g. 0.40)
...
0.75 # Control rule inflection for constant F (as frac of Bzero, e.g. 0.40); must be > control rule cutoff, or set to -1 to use Bmsy/SSB_unf 
0.20 # Control rule cutoff for no F (as frac of Bzero, e.g. 0.10) 
...
1979 # Rebuilder:  first year catch could have been set to zero (Ydecl)(-1 to set to 1999)
