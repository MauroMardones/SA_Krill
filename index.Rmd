...
In this approach we include spatial structure how an implicit approach where different areas are treated as separate fleets. Differences among strata have been identified from various perspectives of krill population dynamics, as well as from the influence of environmental variability within Subarea 48.1 (Figure \@ref(fig:concem)).
...
outps1$Model <- "Ref Model: No Env-Predator"
outps2$Model <- "s1.2 w/ Predator data"
outps3$Model <- "s1.3 w/ Env data"
outps4$Model <- "s1.4 w/ Env and Predator data"
...
ggplot(outpsall_long, aes(x = factor(Yr), y = Value, fill = Model)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~Variable, 
             ncol=1,
             scales = "free_y") + 
  scale_fill_manual(name= "Scenario",
                    values = c("Ref Model: No Env-Predator" = "#ca0020",  
                              
                               "s1.4 w/ Env and Predator data" = "#404040")) +
  labs(title = "",
       x = "", y = "") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
