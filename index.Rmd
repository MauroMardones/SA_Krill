...
indres <- ggplot(cpue_resid, aes(x = Exp, y = Obs, color = model)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_wrap(~Fleet_name, scales = "free") +
  theme_minimal() +
  labs(title = "Observed vs Expected (Index)",
       x = "Expected",
       y = "Observed")
...
lenres <- ggplot(len_resid, aes(x = Exp, y = Obs, color = model)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
...
ggplot(resid_all, aes(x = Exp, y = Obs, color = model)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray40") +
  facet_grid(type ~ Fleet_name, scales = "free") +
  theme_minimal(base_size = 13) +
  labs(title = "Observed vs Expected residuals",
       x = "Expected", y = "Observed")

```
```{r}
ggplot(resid_all, aes(x = Obs - Exp, fill = model)) +
  geom_density(alpha = 0.5) +
  facet_wrap(Fleet_name~type, scales = "free") +
  labs(title = "Distribución de residuales", x = "Residual (Obs - Exp)", y = "Densidad") +
  theme_minimal()

```
### Statistics

```{r eval=FALSE, resumen_residuales, message=FALSE, warning=FALSE}
resid_all <- resid_all %>%
...
flextable(tabla_resumen) %>%
  set_caption("Tabla 1. Estadísticos y pruebas sobre residuales por tipo y modelo") %>%
  autofit()
