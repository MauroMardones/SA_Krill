...
ft <- set_caption(ft, "Input parameters for the initial SS3 model of krill. Each parameter line contains a minimum value (LO), maximum value (HI), and initial value (INIT). If the phase (PHASE) for the parameter is negative, the parameter is fixed as input")
ft <- autofit(ft)
...
```{r dataserie, fig.width=9, fig.height=7, fig.cap="Data series used in krill modelling in 48.1 Subarea"}
SSplotDataEnv(base.model1.4, 
           subplots = 1,
           ptsize = -2)
```
...
model_names <- paste0("base.model1.", 1:4)

# Bucle para calcular Pearson residual
for (mn in model_names) {
  # Obtener el objeto del modelo
  mod <- get(mn)
  
  # Calcular residual de Pearson
  mod$cpue$PearsonRes <- with(mod$cpue, (Obs - Exp) / SE_input)
  
  # Guardar de vuelta en el entorno global
  assign(mn, mod)
}

# Revisar el resultado, ejemplo para el primero
head(base.model1.1$cpue[, c("Obs", "Exp", "SE_input", "PearsonRes")])
...
# Crear un data.frame combinado con todos los modelos
cpue_all <- bind_rows(
  lapply(model_names, function(mn) {
    mod <- get(mn)
    df <- mod$cpue
    df$Model <- mn  # añadir columna con el nombre del modelo
    return(df)
  })
)

# Plot de residuos de Pearson por año, por flota, facetado por modelo
ggplot(cpue_all, aes(x = Yr, y = PearsonRes)) +
  geom_point(alpha = 0.6) +
  facet_grid(Model ~ Fleet_name) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  theme_minimal() +
  labs(
    title = "Residuos de Pearson por modelo y flota",
    x = "Año",
