...
dir8<-here("s8")
Figs <- here("Figs")# S2 set parametres EMM-2024/23 (Mardones)
```
...
directorios <- c("s01", 
                 ...
                 "s6",
                 "s7",
                 "s8")  # Agrega aquí todos los nombres de las carpetas que deseas procesar
...
SSplotMnwt(base.model01, subplots = 2)
```

## Relationship Stock-Recruit

```{r eval=FALSE}
l <- base.model4$recruit$SpawnBio
m <- base.model4$recruit$pred_recr
ggplot()+
  aes(l,m)+
  geom_point()+
  geom_smooth(method = "lm", 
              se =F)+
  ylim(0, max(base.model2$recruit$pred_recr))
```
...
like8 <- base.model8$likelihoods_used
like8$model <- rep("s8", nrow(like8))
like8 <- rownames_to_column(like8, var = "Description")
...
comp2_8 <- SSplotComparisons(summaryoutput,
                  legendlabels = c(
                                   "s8",
...
```{r}
sblt01 <- base.model01$timeseries$Bio_all 
...
sblt8 <- base.model8$timeseries$Bio_all
...
recs8 <- base.model8$recruit$pred_recr
...
p7 <- ggAcf(base.model7$recruit$pred_recr) +
  ggtitle("S6") +
  theme_few()
p8 <- ggAcf(base.model8$recruit$pred_recr) +
  ggtitle("S6") +
  theme_few()
...
dev7 <- base.model7$recruit[1:43,c(1,7)] %>% 
  mutate(Serie = "S7")
dev8 <- base.model8$recruit[1:43,c(1,7)] %>% 
  mutate(Serie = "S8")

data_list <- list(dev01, dev1, dev2, dev3, dev4, dev5, dev6, dev7, dev8)
titles <- c("S01", "S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8")
# Lista para almacenar los gráficos
plots <- list()

# Crear los gráficos en un ciclo for
...
combined_plot_join <- ggplot(combined_data, 
                             aes(x = Yr, 
                                 y = dev, 
                                 color = Serie)) +
  geom_point() +
